...
const REGENERATIONPAGE_QUERY = gql`
  {
    site: _site {
...
class AboutLayout extends React.Component {
  ...
  render() {
    ...
    return (
      ...
      >
        {data.regenerationPage.content.map((o, i) => {
          const id = "id" + o.id;
          const style = {};
    ...
    return (
      <div className={styles.container}>
        <div className={styles.spaced}>
          <Query query={REGENERATIONPAGE_QUERY}>
            {({ loading, error, data }) => {
              if (loading)
              ...
              return (
                <>
                  <Head>{renderMetaTags([...data.regenerationPage.seo, ...data.site.favicon])}</Head>
